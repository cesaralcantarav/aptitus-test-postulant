...
def get_env_and_sub_env(enviroment):
    return enviroment.split('.')
...
def command(ctx, **kwargs):
    ...
    try:
        (env, sub_env) = get_env_and_sub_env(kwargs['env'])
        ...
        endpoint = get_endpoint_postulant(ctx.config, env)
        endpoint = endpoint.format(test_env=kwargs['env'])
